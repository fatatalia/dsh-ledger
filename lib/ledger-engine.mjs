...
export function summarizeBalances(accounts) {
  ...
  for (const { account, amount } of accounts) {
    ...
    if (amt === 0) continue; // 已赎回/关闭的 0 余额账户不展示（如已清空的理财子账户）
    totalAssets += amt;
    ...
    g.amount += amt;
    g.accountCount += 1;
    g.accounts.push({ account, amount: amt });
    groups.set(top, g);
    if (parts.length === 3 && /^\d+$/.test(parts[2])) liquid += amt; // 数字尾号 = 银行卡
  }
  const groupList = [...groups.entries()]
    .map(([name, g]) => ({ name, amount: round2(g.amount), accountCount: g.accountCount, accounts: g.accounts }))
    .filter((g) => g.accountCount > 0) // 全组 0 余额（全部关闭）则不展示该分组
